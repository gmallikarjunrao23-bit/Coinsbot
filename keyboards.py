...
def main_menu(is_admin: bool):
    rows = [
        [btn("🏪 Store", "primary"), btn("👤 Profile", "primary")],
        [btn("🎫 Redeem Code", "primary"), btn("👥 Referrals", "primary")],
        [btn("🎁 Daily Bonus", "success"), btn("🏆 Leaderboard", "primary")],
    ]
    if is_admin:
...
def category_menu(buttons):
    ...
    rows.append([btn("🛒 My Purchases", "success")])
    rows.append([btn("🔙 Back", "primary")])
    return _kb(rows, "Select a category")


def admin_menu():
    rows = [
        [btn("📊 Stats", "primary"), btn("📦 Add Content", "success")],
        [btn("📋 Manage Content", "primary"), btn("🎫 Gift Codes", "success")],
        [btn("➕ Add Coins", "success"), btn("➖ Remove Coins", "danger")],
        [btn("👥 Users", "primary"), btn("📢 Broadcast", "primary")],
...
def back_to_store():
    return _kb([[btn("🔙 Back", "primary")]], "Type the item number to purchase")
