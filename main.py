...
def header(emoji: str, title: str) -> str:
    """Clean, modern section header — replaces the old heavy block-border
    style with a single bold line + thin divider for a more premium feel."""
...
def category_buttons():
    ...
    return [f"{category_icon(t)} {t.upper()}" for t in db.get_distinct_types()]
...
async def text_router(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ---------------- ADMIN MENU ----------------
        ...
        await update.message.reply_text(
            header("👑", "ADMIN CONTROL PANEL"), reply_markup=kb.admin_menu(), parse_mode="Markdown"
        )
    ...
    # (removed — 🎫 GIFT CODES now goes straight to the unified create flow)
        ...
        for t in db.get_distinct_types():
            if text == f"{category_icon(t)} {t.upper()}":
                matched_category = t
                break
        if matched_category:
