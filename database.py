...
class Database:
        ...
        with self._conn() as c:
            ...
            cur.execute("""
                CREATE TABLE IF NOT EXISTS content (
                    ...
                    title TEXT,
                    description TEXT,
                    type TEXT CHECK(type IN ('course','file','method')),
                    price INTEGER,
                    file_id TEXT,
                    file_name TEXT,
                    created_at TIMESTAMP,
    ...
    # ---------------- CONTENT METHODS ----------------
    def add_content(self, title, description, ctype, price, file_id, file_name):
        with self._conn() as c:
            cur = self._cursor(c)
            cur.execute("""
                INSERT INTO content (title, description, type, price, file_id, file_name, created_at, active, total_sold)
                VALUES (%s, %s, %s, %s, %s, %s, %s, 1, 0)
                RETURNING id
            """, (title, description, ctype, price, file_id, file_name, now()))
    ...
    # ---------------- GIFT CODE METHODS ----------------
    def generate_gift_code(self, coins, created_by):
        code = "".join(random.choices(string.ascii_uppercase + string.digits, k=10))
        with self._conn() as c:
            cur = self._cursor(c)
            cur.execute("""
                INSERT INTO gift_codes (code, coins, claimed_by, created_by, created_at, used_at, is_active)
                VALUES (%s, %s, 0, %s, %s, NULL, 1)
            """, (code, coins, created_by, now()))
            cur.close()
        return code
    ...
    def redeem_gift_code(self, code, user_id):
        with self._conn() as c:
            ...
            if row["is_active"] == 0 or row["claimed_by"] != 0:
                cur.close()
                return None, "used"
            cur.execute("""
                UPDATE gift_codes SET is_active=0, claimed_by=%s, used_at=%s WHERE code=%s
            """, (user_id, now(), code))
            coins = row["coins"]
            cur.close()
