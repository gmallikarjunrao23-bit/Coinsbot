...
def header(emoji: str, title: str) -> str:
    """Clean, professional section header — a single leading icon with
    Title Case text and a thin divider, avoiding ALL CAPS / stacked emoji."""
...
# Smart icon matching so custom admin-defined categories still look curated
# instead of a generic box for everything.
CATEGORY_ICONS = [
    (("ebook", "book", "novel"), "📖"),
    (("script", "code", "bot", "software"), "💻"),
    (("course", "class", "tutorial"), "🎓"),
    (("template", "design", "ui"), "🎨"),
    (("wallpaper", "image", "photo"), "🖼️"),
    (("method", "guide", "strategy"), "🧭"),
    (("video", "movie", "film"), "🎬"),
    (("music", "audio", "sound"), "🎵"),
    (("game", "gaming"), "🎮"),
    (("file", "doc", "pdf"), "📁"),
...
async def send_main_menu(update: Update, text: str):
    ...
    dashboard = ""
    if u:
        dashboard = f"\n\n┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄\n🪙 {fmt_coins(u['coins'])}   👥 {u['referral_count']} referrals"
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    welcome = (
        header("💎", "Premium Vault") +
        f"👋 Welcome, *{user.first_name}*!\n\n"
        "Explore the store, earn coins, and unlock premium content 🚀"
    )
...
async def show_leaderboard(update: Update):
    ...
    await update.message.reply_text(header("🏆", "Top 10 Leaderboard") + "\n".join(lines) + FOOTER, parse_mode="Markdown")
...
def category_buttons():
    """Builds the labeled category buttons using smart per-category icons."""
    return [f"{category_icon(t)} {t.title()}" for t in db.get_distinct_types()]


async def show_store(update: Update):
    text = (
        header("💎", "Premium Store") +
        "Browse everything available right now — pick a category below:"
    )
    await update.message.reply_text(text + FOOTER, reply_markup=kb.category_menu(category_buttons()), parse_mode="Markdown")


async def show_content_list(update: Update, ctype: str):
    items = db.get_content_by_type(ctype)
    icon = category_icon(ctype)
    lines = []
    if not items:
        ...
        for it in items:
            lines.append(
                f"✨ *{it['title']}*  `#{it['id']}`\n"
                f"📝 {it['description']}\n"
                f"🪙 {fmt_coins(it['price'])}   🔥 {it['total_sold']} sold\n"
                f"👉 Type `{it['id']}` to buy\n"
                f"┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄"
            )
    await update.message.reply_text(
        header(icon, ctype.title()) + "\n".join(lines) + FOOTER,
        reply_markup=kb.back_to_store(), parse_mode="Markdown"
    )
...
async def show_my_purchases(update: Update):
    ...
    await update.message.reply_text(
        header("🛒", "My Purchases") + "\n".join(lines) + FOOTER,
        reply_markup=kb.category_menu(category_buttons()), parse_mode="Markdown"
    )
...
async def perform_purchase(update: Update, context: ContextTypes.DEFAULT_TYPE, user_id: int, item: dict):
    ...
    if u["coins"] < item["price"]:
        await update.message.reply_text(
            ...
            "Earn more via 🎁 Daily Bonus or 👥 Referrals!",
            parse_mode="Markdown",
        )
        return
...
async def confirm_buy_process(update: Update, context: ContextTypes.DEFAULT_TYPE, text: str):
    ...
    if text != "✅ Confirm" or not content_id:
...
async def add_content_price(update: Update, context: ContextTypes.DEFAULT_TYPE, text: str):
    ...
    await update.message.reply_text(
        "*Step 5/5* — Now send the content itself. This can be a *file* "
        "(document, PDF, ZIP, photo, video, or audio), *or* you can just "
        "type/paste the content directly as *text* (great for guides, "
        "methods, or codes).",
        reply_markup=kb.cancel_menu(), parse_mode="Markdown",
    )
...
async def admin_users(update: Update):
    ...
    lines = [f"(showing latest {len(users)})\n"]
    for u in users:
...
async def text_router(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # If admin is mid-way through the final content step, plain text is
    # accepted as the content itself (e.g. a written guide or method).
    ...
    # ---------------- ADMIN MENU ----------------
        ...
        if text.isdigit():
            ...
            if item and item["active"]:
                ...
                confirm_text = (
                    header("🛍️", "Confirm Purchase") +
                    f"📦 *{item['title']}*\n"
                    f"📝 {item['description']}\n\n"
                    f"🪙 Price: {fmt_coins(item['price'])}\n"
                    f"🪙 Your Balance: {fmt_coins(buyer['coins'])}\n\n"
                    "Tap *✅ Confirm* to complete the purchase, or *❌ Cancel*."
                )
                await update.message.reply_text(
                    confirm_text, reply_markup=kb.confirm_menu("✅ Confirm", "❌ Cancel"), parse_mode="Markdown"
                )
                return
            else:
        ...
        for t in db.get_distinct_types():
            if text == f"{category_icon(t)} {t.title()}":
                matched_category = t
                break
        if matched_category:
